//
//  BotsiPaywallViewModifier.swift
//
//
...
@MainActor
struct BotsiPaywallViewModifier<AlertItem>: ViewModifier where AlertItem: Identifiable {
    @Environment(\.presentationMode) private var presentationMode

    private let isPresented: Binding<Bool>
    private let fullScreen: Bool

    private let paywallConfiguration: BotsiUI.PaywallConfiguration
    ...
    init(
        ...
        paywallConfiguration: BotsiUI.PaywallConfiguration,
...
public extension View {
    ...
    ///     - paywallConfiguration: an ``BotsiUI.PaywallConfiguration`` object containing information about the visual part of the paywall. To load it, use the ``BotsiUI.paywallConfiguration(for:products:viewConfiguration:observerModeResolver:tagResolver:timerResolver:)`` method.
    ...
    func paywall<AlertItem: Identifiable>(
        ...
        fullScreen: Bool = true,
        paywallConfiguration: BotsiUI.PaywallConfiguration,
        didPerformAction: ((BotsiUI.Action) -> Void)? = nil,
        didSelectProduct: ((BotsiProduct) -> Void)? = nil,
        didStartPurchase: ((BotsiPaywallProduct) -> Void)? = nil,
        didFinishPurchase: ((BotsiPaywallProduct, BotsiPurchaseResult) -> Void)? = nil,
        didFailPurchase: @escaping (BotsiPaywallProduct, BotsiError) -> Void,
        didStartRestore: (() -> Void)? = nil,
        didFinishRestore: @escaping (BotsiProfile) -> Void,
        didFailRestore: @escaping (BotsiError) -> Void,
        didFailRendering: @escaping (BotsiError) -> Void,
        didFailLoadingProducts: ((BotsiError) -> Bool)? = nil,
        showAlertItem: Binding<AlertItem?> = Binding<BotsiIdentifiablePlaceholder?>.constant(nil),
        ...
        return modifier(
            BotsiPaywallViewModifier<AlertItem>(
                isPresented: isPresented,
