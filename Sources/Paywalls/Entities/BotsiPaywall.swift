...
public struct BotsiPaywall: Sendable {
    /// An identifier of a placement, configured in Botsi Dashboard.
    ...
    public let remoteConfig: BotsiPaywall.RemoteConfig?

    /// If `true`, it is possible to fetch the view ``BotsiViewConfiguration`` object and use it with ``BotsiUI`` library.
...
extension BotsiPaywall: CustomStringConvertible {
...
extension BotsiPaywall: ValueHashable {}

extension BotsiPaywall: Codable {
...
extension Sequence<VH<BotsiPaywall>> {
    var asPaywallByPlacementId: [String: VH<BotsiPaywall>] {
