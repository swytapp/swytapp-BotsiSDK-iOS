//
//  BotsiPaywall.FetchPolicy.swift
//  BotsiSDK
//
...
extension BotsiPaywall.FetchPolicy {
    func canReturn(_ data: VH<BotsiPaywall>) -> Bool {
        switch self {
...
extension VH<BotsiPaywall> {
    @inlinable
    func withFetchPolicy(_ fetchPolicy: BotsiPaywall.FetchPolicy) -> Self? {
        fetchPolicy.canReturn(self) ? self : nil
    }
}

extension BotsiPaywall.FetchPolicy: Codable {
