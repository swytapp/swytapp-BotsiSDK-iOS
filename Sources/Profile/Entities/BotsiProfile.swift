...
public struct BotsiProfile: Sendable {
    /// An identifier of a user in Botsi.
    ...
    let codableCustomAttributes: BotsiProfile.CustomAttributes?

    /// Previously set user custom attributes with `.updateProfile()` method.
    public let customAttributes: [String: any Sendable]

    /// The keys are access level identifiers configured by you in Botsi Dashboard. The values are Can be null if the customer has no access levels.
...
extension BotsiProfile: CustomStringConvertible {
...
extension BotsiProfile: ValueHashable {}

extension BotsiProfile: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        codableCustomAttributes = try container.decodeIfPresent(BotsiProfile.CustomAttributes.self, forKey: .customAttributes)
