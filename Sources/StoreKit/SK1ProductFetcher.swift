...
private final class _SK1ProductFetcher: NSObject, @unchecked Sendable {
    private let queue = DispatchQueue(label: "Botsi.SDK.SK1ProductFetcher")
    private var requests = [Int: (productIds: Set<String>, retryCount: Int)]()
    private var completionHandlers = [Set<String>: [BotsiResultCompletion<[SK1Product]>]]()

    func fetchProducts(productIdentifiers productIds: Set<String>, retryCount: Int = 3, _ completion: @escaping BotsiResultCompletion<[SK1Product]>) {
        queue.async { [weak self] in
            guard let self else {
                completion(.failure(StoreKitManagerError.interrupted().asBotsiError))
                return
            }
...
extension _SK1ProductFetcher: SKProductsRequestDelegate {
    ...
    func request(_ request: SKRequest, didFailWithError error: Error) {
        ...
        queue.async { [weak self] in
            ...
            let error = StoreKitManagerError.requestSK1ProductsFailed(error).asBotsiError
            for completion in handlers {
