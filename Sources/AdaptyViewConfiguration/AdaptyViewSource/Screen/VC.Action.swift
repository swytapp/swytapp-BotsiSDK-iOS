...
extension BotsiViewSource {
    enum Action: Sendable {
        case openUrl(String)
        case action(BotsiViewConfiguration.ActionAction)
    }
}

extension BotsiViewSource.Localizer {
    func action(_ from: BotsiViewSource.Action) throws -> BotsiViewConfiguration.ActionAction {
        switch from {
...
extension BotsiViewSource.Action: Hashable {
...
extension BotsiViewSource.Action: Decodable {
    ...
    package init(from decoder: Decoder) throws {
        ...
        case .purchaseSelectedProduct:
            self = try .action(.purchaseSelectedProduct(
                groupId: container.decodeIfPresent(String.self, forKey: .groupId) ?? BotsiViewSource.StringId.Product.defaultProductGroupId
            ))
