        ...
        init(from decoder: Decoder) throws {
            ...
            content =
                if
                    ...
                    try container.decodeIfPresent(String.self, forKey: .version).map(BotsiViewConfiguration.formatVersion.isSameOrNewerVersion) ?? true {
                    try container.decode(BotsiViewSource.Element.self, forKey: .then)
                } else {
                    try container.decode(BotsiViewSource.Element.self, forKey: .else)
                }
        }
    }
}
