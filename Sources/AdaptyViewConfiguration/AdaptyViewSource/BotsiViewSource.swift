...
struct BotsiViewSource: Sendable, Hashable {
    let id: String
    let templateId: String
    let templateRevision: Int64
    let assets: [String: Asset]
    let responseLocale: BotsiLocale
    let localizations: [BotsiLocale: Localization]
...
extension BotsiViewSource: CustomStringConvertible {
    var description: String {
        "(id: \(id), templateId: \(templateId), templateRevision: \(templateRevision))"
    }
}

extension BotsiViewSource: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        responseLocale = try superContainer.decode(BotsiLocale.self, forKey: .responseLocale)
        ...
        if container.contains(.products) {
            ...
            if let selected = try? products.decodeIfPresent(String.self, forKey: .selected) {
                selectedProducts = [BotsiViewSource.StringId.Product.defaultProductGroupId: selected]
            } else {
        ...
        let localizations = try [BotsiLocale: Localization](localizationsArray.map { ($0.id, $0) }, uniquingKeysWith: { _, _ in
        ...
        if let defaultLocalization = try container.decodeIfPresent(BotsiLocale.self, forKey: .defaultLocalization) {
